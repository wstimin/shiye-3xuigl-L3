...
test('reference binding is rejected before debit or remote write', async () => {
  const fixture = renewalFixture({ node: { remoteControl: 'reference' } });
  await assert.rejects(
    () => fixture.service.renewCustomerNode('customer-1', 'customer-node-1', 1, 'admin', '33333333-3333-4333-8333-333333333333'),
    /只读引用/
  );
  assert.equal(fixture.debitCount, 0);
  assert.equal(fixture.remoteWrites.length, 0);
